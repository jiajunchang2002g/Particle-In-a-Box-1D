...
def f(x, t):
    ...
    for i in range (1, N+1):
        ...
        ui = u(odd_i, x)
        vi = v(even_i, x)
        sum += np.abs(ci)**2 * ui**2 + np.abs(di)**2 * vi**2 - 2 * ci * abs(di) * ui * vi * np.sin( (even_i + odd_i) * w0 * t)
        for j in range (1, i):
            odd_j = 2 * j - 1
            even_j = 2 * j
            cj = cn[odd_j]
...
def animate():
    ...
    def init():
        ...
        return line,
    def update(frame):
        t = frame * 0.5  # Adjust the speed of the animation by changing the factor here
